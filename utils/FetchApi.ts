...
export const fetchGetAllTickers = async (): Promise<ITicker[]> => {
  return fetch(`${process.env.BASE_FETCH_URL}/api/tickers`, { next: { revalidate: 60 } })
    .then(res => res.json())
    .then(res => res.data)
}
...
export const fetchGetTickerDetail = async ({ coin }: IRequest): Promise<ITicker[]> => {
  if (coin) {
    return fetch(`${process.env.BASE_FETCH_URL}/api/tickers/search?coin=${coin}`)
      .then(res => res.json())
...
export const fetchGetMarketByCoin = async ({ id }: IRequestMarket): Promise<IMarket[]> => {
  return fetch(`${process.env.BASE_FETCH_URL}/api/markets?coin=${id}`)
    .then(res => res.json())
